    ...
    static void CreateScriptableObject()
    {
        ...
        if (obj != null && obj.GetType() == typeof(MonoScript))
        {
            ...
            if (script != null && script.GetClass().IsSubclassOf(typeof(ScriptableObject)))
            {
                ...
                if (isInRenameMode)
                {
                    ProjectWindowUtil.CreateAsset(asset, $"Assets/ScriptableObjects/{script.name}.asset");
                    Selection.activeObject = obj;
                }
                else
                {
                    AssetDatabase.CreateAsset(asset, $"Assets/ScriptableObjects/{script.name}.asset");
